...
function CreateEmployee(props: any) {
  ...
  const [alltemplatename, setAllTemplateName] = useState([]); // State to store selected options
  ...
  async function gettemplatedata(templatename: string) {
    if (templatename == "") setTemplatedata([]);
    else {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/mg_salary_template/${templatename}`,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (response.status === 200) {
          // console.log(response.data, "all earning data");
          setAllTemplateName(response.data);
        }
      } catch (error) {
        console.error(error);
      }
